...
class MyW2V2(chainer.Chain):
    ...
    def __call__(self, xb, eb, sampler, SAMPLING_SIZE):
        loss = None        
        for i in range(len(xb)):
            x = Variable(np.array([xb[i]], dtype=np.int32))
            e = eb[i]
            ls = F.negative_sampling(e, x, self.embed.W, sampler, SAMPLING_SIZE)
...
for epoch in range(10):
    ...
    for pos in range(0, datasize, bs):    
        ...
        model.zerograds()        
        loss = model(xb, eb, sampler.sample, SAMPLING_SIZE)
        loss.backward()
        optimizer.update()
